// students.js - VERSION 2.0
// ======================= DROPDOWN KELAS & JURUSAN DINAMIS =======================
// Dengan real-time update dan auto-sync
        ...
        tbody.innerHTML += `
            <tr class="${isNew ? 'student-new-row' : ''}" data-id="${s.id}">
                ...
                <td>
                    ...
                    <button class="btn-icon delete" onclick="deleteStudent('${s.id}')" title="Hapus Siswa">🗑️</button>
                 </td>
             </tr>`;
    });
...
function saveStudent() {
    ...
    // Cek duplikasi ID untuk mode add
...
function resetStudentForm() {
    ...
    // Focus ke ID field setelah reset
...
function deleteStudent(id) {
    ...
    const student = dbData.users.find(u => u.id == id);
    const studentName = student?.nama || id;
    
    if (!confirm(`⚠️ Yakin ingin menghapus siswa "${studentName}"?\n\nData absensi siswa ini juga akan terpengaruh!`)) return;
    
    // Cek apakah siswa sudah memiliki akun
    const registeredUser = dbData.users_auth?.find(u => u.fpId == id);
    if (registeredUser) {
        if (!confirm(`⚠️ Siswa ini sudah memiliki akun (${registeredUser.email}).\n\nHapus juga akun pengguna?`)) {
            return;
        }
        // Hapus akun auth juga
        db.ref(`users_auth/${registeredUser.uid}`).remove()
            .then(() => console.log("✅ User auth removed"))
            .catch(err => console.warn("Gagal hapus user auth:", err));
    }
    
    const btn = document.querySelector(`button[onclick="deleteStudent('${id}')"]`);
    if (btn) btn.disabled = true;
    
    db.ref(`users/${id}`).remove()
        .then(() => {
            showToast(`✅ Siswa "${studentName}" berhasil dihapus`);
        })
        .catch((err) => {
...
// Auto-init realtime students saat DOM siap
