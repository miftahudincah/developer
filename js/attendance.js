function renderTable() {
    ...
    // LOGIKA FILTER SISWA (HANYA LIHAT DATA SENDIRI)
    if (currentUser.role === 'siswa') {
        if (currentUser.kelas && currentUser.jurusan) {
            // Filter ketat: Kelas DAN Jurusan harus sama
            data = data.filter(r => 
                r.kelas === currentUser.kelas && 
                r.jurusan === currentUser.jurusan
            );
        } else {
            // Jika data kelas/jurusan di profil kosong, tampilkan array kosong
            data = []; 
        }
    ...
    // LOGIKA FILTER GURU & ADMIN (LIHAT SEMUA / BISA FILTER MANUAL)
    else {
        if (fDate === 'today') {
            const todayStr = new Date().toISOString().split('T')[0];
            data = data.filter(r => r.date === todayStr);
    ...
    tbody.innerHTML = '';
    if (data.length === 0) { 
        tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding:20px;">Data absensi tidak ditemukan.</td></tr>'; 
        return; 
    }
...
function deleteAttendance(id) {
    ...
    db.ref(`absensi/${date}/${fpId}`).remove().then(() => showToast("Data dihapus"));
}
...
function simulateAttendance() {
    ...
    // Simulasi data masuk ke Firebase seolah-olah dari ESP32
    db.ref(`absensi/${dateStr}/${s.id}`).set({
        nama: s.nama, 
        kelas: s.kelas, 
        jurusan: s.jurusan, 
        in: timeStr
    }).then(() => showToast(`Simulasi Scan: ${s.nama}`));
}
